...
import { MODES } from './modes.js?v=2.1.0';
import { assessReadings } from './quality.js?v=2.1.0';
import { analyseGreyscale, analyseGamut, scorecard } from './analysis.js?v=2.1.0';
...
export function makeReport(data) {
  ...
  try { if (kinds.has('grey')) grey = analyseGreyscale(readings, target); }
  ...
  if (t.exposureLocked === false) { score = Math.min(score, 69); quality.warnings.push('Exposure lock was not confirmed by this camera. Stable ratios cannot remove all automatic camera processing.'); }
  if (t.whiteBalanceLocked === false) { score = Math.min(score, 79); quality.warnings.push('White balance lock was not confirmed. Colour and channel-drift results remain approximate.'); }
  ...
  const notMeasured = (key, included) => data.omissions?.[key] || (included ? 'Not measured: the required readings are missing or the test was interrupted.' : `Not measured in ${name}.`);
  const coverage = [
  ...
  if (grey && Number.isFinite(grey.gamma)) {
  ...
  return { target, mode, grey, gamut, quality, grade, score, repeat, coverage, recommendations,
