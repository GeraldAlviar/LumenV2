...
import { abortError } from './async.js?v=2.1.0';
import { validMessage, MAX_SESSION_BYTES } from './validation.js?v=2.1.0';
  ...
  _bind(conn) {
    ...
    conn.on('data', raw => {
      ...
      try {
        ...
        if (!validMessage(msg)) {
          if (Number.isSafeInteger(msg?.__id) && msg.__id > 0)
            this.reply(msg, { t: 'error', error: 'Incompatible or invalid message. Refresh BOTH devices to the same Lumen version.' });
          return;
        }
      } catch { return; }
      this._lastSeen = Date.now();
