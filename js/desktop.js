...
import { createLink, makeRoomCode } from './net.js?v=2.2.0';
import { hex, clamp } from './colour.js?v=2.2.0';
import { TARGETS } from './patches.js?v=2.2.0';
import { analyseGreyscale, analyseGamut, scorecard, buildCorrectionLut } from './analysis.js?v=2.2.0';
import { buildIccProfile, buildArgyllCal, buildCsv, download } from './icc.js?v=2.2.0';
import { assessReadings } from './quality.js?v=2.2.0';
import { validateResults, MAX_SESSION_BYTES } from './validation.js?v=2.2.0';
import { sessionBlob, parseSession, saveLastSession, SESSION_KEY } from './session.js?v=2.2.0';
import { syntheticReadings } from './demo.js?v=2.2.0';
import { DisplayStage } from './display.js?v=2.2.0';
import { BUILD, PROTOCOL, STAGES, formatTime } from './config.js?v=2.2.0';
import { makeReport } from './report.js?v=2.2.0';
import { verifyResume, MODES, planFor, PLAN_VERSION } from './modes.js?v=2.2.0';
import { createSession } from './session.js?v=2.2.0';
...
const state = { purpose: 'assessment', capture: { diagnosticOnly: false, warnings: [] }, comparison: null, displaySetup: { controls: [] },
  compatible: false, backup: null, run: undefined, planVersion: PLAN_VERSION, phase: 'idle', reason: '', telemetry: {}, omissions: {}, baseline: null, alignment: null, elapsedMs: 0, revision: -1, lastActivity: 0,
...
function stageStatus(phase, label, progress) {
  ...
  $('#stage-hud').classList.toggle('attention', ['paused', 'stopped', 'error', 'review'].includes(phase));
  $('#stage-back').classList.toggle('hidden', !['paused', 'stopped', 'error', 'review'].includes(phase));
  if (progress) {
    ...
    $('#stage-eta').textContent = phase === 'review' ? 'Choose on phone' : ['paused', 'stopped'].includes(phase) ? 'Paused' : formatTime(progress.remaining) + ' left';
...
async function handleMessage(msg) {
  switch (msg.t) {
    ...
    case 'target':
      ...
      break;
    case 'run-start':
      if (!MODES[msg.run] || planFor(msg.run, msg.purpose).length !== msg.expected) throw new Error('The test plan does not match this build. Refresh both devices.');
      ...
      state.target = TARGETS[msg.target]; state.expected = msg.expected; state.run = msg.run; state.purpose = msg.purpose || 'assessment';
      ...
      if (!state.backup || state.backup.sessionId !== msg.sessionId) {
        state.backup = createSession({ sessionId: msg.sessionId, target: msg.target, run: msg.run, purpose: msg.purpose || 'assessment', planVersion: PLAN_VERSION, readings: [], expected: msg.expected, phase: 'starting', complete: false });
...
function snapshot() {
  return { target: state.target.id, readings: state.readings, uniformity: state.uniformity, flicker: state.flicker,
    ...
    ...(state.run ? { run: state.run, planVersion: PLAN_VERSION } : {}), phase: state.phase, reason: state.reason, telemetry: state.telemetry,
    omissions: state.omissions, baseline: state.baseline, alignment: state.alignment, elapsedMs: state.elapsedMs, purpose: state.purpose, capture: state.capture, comparison: state.comparison, displaySetup: state.displaySetup };
}
function resetReport() {
  ...
  state.capture = { diagnosticOnly: false, warnings: [] }; state.comparison = null; state.purpose = 'assessment'; state.displaySetup = { controls: [] };
...
function ingestResults(message) {
  ...
  state.capture = msg.capture || { diagnosticOnly: false, warnings: [] }; state.comparison = msg.comparison || null; state.purpose = msg.purpose || 'assessment'; state.displaySetup = msg.displaySetup || { controls: [] };
...
function renderReport() {
  ...
  $('#report-label').textContent = state.synthetic ? 'SIMULATED DATA / DEMO' : state.capture.diagnosticOnly ? 'DIAGNOSTIC REPORT / WARNINGS ACCEPTED' : 'Experimental camera measurement';
  ...
  $('#report-coverage').replaceChildren(...state.report.coverage.map(c => { const box = el('div', 'coverage-item' + (c.measured ? '' : ' not-measured')), title = el('strong', null, c.name), status = el('span', 'pill ' + (c.measured ? 'warning' : 'estimate'), c.status || (c.measured ? 'Camera-relative estimate' : 'Not measured')); box.append(title, status, el('p', 'small dim', c.note)); return box; }));
  const comparison = state.report.comparison;
  $('#report-comparison').textContent = comparison ? comparison.status + '. ' + comparison.message + (Number.isFinite(comparison.beforeError) ? ` Before: ${comparison.beforeError.toFixed(2)} pp; after: ${comparison.afterError.toFixed(2)} pp.` : '') : 'No fresh before/after check yet. Make any changes yourself, then start verification on the phone. Lumen does not install a profile.';
  ...
  for (const row of rows) {
    ...
    const td = el('td'), status = state.quality.unreliable.length || state.capture.diagnosticOnly || state.telemetry.exposureLocked !== true ? 'estimate' : row.status;
...
function updateExports() {
  ...
  $('#export-note').textContent = !canExport ? 'Correction files require complete usable data, confirmed exposure and white-balance locks, and no diagnostic override. See the quality notes above.'
