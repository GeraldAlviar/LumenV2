...
async function preflight(session, signal, resume = false) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    result = await sensor.measure({ settleMs: 900, minSamples: 18, maxMs: 2800, tolerance: 0.025, signal });
  ...
  result = await sensor.measure({ settleMs: 900, minSamples: 24, maxMs: 3200, tolerance: 0.02, signal });
  ...
  const low = await sensor.measure({ settleMs: 700, minSamples: 18, maxMs: 2600, tolerance: 0.025, signal });
...
const runner = new TestRunner({
  ...
  }, preflight, review: reviewCapture, render, measure: options => state.sensor.measure(options), save,
...
function help(reason, hasSession = Boolean(runner.session), error) {
  ...
  const title = error?.issues?.[0]?.title || (/clipp|overexposed/i.test(reason) ? 'Camera sample is overexposed' : /marker/i.test(reason) ? 'Movement markers are hidden' : /mov|align/i.test(reason) ? 'Reposition the samples' : /frame|camera|playback/i.test(reason) ? 'Camera frames are unavailable' : /connect|monitor|acknowledge/i.test(reason) ? 'Monitor connection needs a retry' : 'Test paused: action required');
  ...
  $('#help-detail').textContent = /clipp|overexposed/i.test(reason)
...
window.lumen = { state, link, runner, store, preflight, showAlignment, lockSampling, reviewCapture };
