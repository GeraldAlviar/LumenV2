...
const kinds = new Set(['grey', 'ladder', 'flare', 'gamut', 'memory', 'repeat', 'reference', 'verification']);
...
export function validReading(r) {
  ...
  return kinds.has(p.kind) && text(p.label) && triple(p.test, 0, 1) &&
    ...
    (!['grey', 'ladder', 'flare', 'verification'].includes(p.kind) || finite(p.level, 0, 1)) &&
...
export function validateResults(m) {
  ...
  if (m.planVersion !== undefined && ![1, 2].includes(m.planVersion)) throw new Error('Unsupported test plan.');
  if (m.purpose !== undefined && !['assessment', 'verification'].includes(m.purpose)) throw new Error('Invalid test purpose.');
  if (m.capture !== undefined && !validCapture(m.capture)) throw new Error('Invalid capture warnings.');
  if (m.displaySetup !== undefined && !validDisplaySetup(m.displaySetup)) throw new Error('Invalid monitor setup.');
  if (m.comparison != null && !validComparison(m.comparison)) throw new Error('Invalid comparison baseline.');
...
export function validTelemetry(t) {
  return record(t) && Object.keys(t).length <= 20 && Object.entries(t).every(([key, value]) => {
    if (['exposureLocked', 'whiteBalanceLocked', 'motionEnabled', 'visualTracking', 'trackingDisabled'].includes(key)) return typeof value === 'boolean';
    return ['retries', 'movements', 'disconnects', 'resumes', 'clipping', 'maxDrift', 'storageFailures', 'referenceDrift', 'referenceChecks'].includes(key) && finite(value, 0, 1e9);
  });
}
...
export function validMessage(m) {
  ...
  switch (m.t) {
    case 'render': return triple(m.test, 0, 1) && text(m.label ?? '') &&
      ...
      (m.layout === undefined || ['field', 'alignment', 'adjustment'].includes(m.layout)) &&
    ...
    case 'run-start': return text(m.sessionId, 100) && TARGET_IDS.includes(m.target) &&
      ...
      (m.purpose === undefined || ['assessment', 'verification'].includes(m.purpose));
...
export function validCapture(c) {
  return record(c) && typeof c.diagnosticOnly === 'boolean' && Array.isArray(c.warnings) && c.warnings.length <= 64 &&
    c.warnings.every(w => record(w) && text(w.code, 80) && text(w.title, 150) && text(w.detail, 500) && text(w.stage, 80) && typeof w.accepted === 'boolean');
}
export function validDisplaySetup(s) {
  return record(s) && Array.isArray(s.controls) && s.controls.length <= 6 && s.controls.every(c => ['brightness', 'contrast', 'temperature', 'rgb', 'gamma', 'none'].includes(c));
}
export function validComparison(c) {
  return record(c) && text(c.sourceSessionId, 100) && text(c.createdAt, 80) && TARGET_IDS.includes(c.target) &&
    ['recheck', 'monitor', 'profile'].includes(c.action) && text(c.note, 500) && record(c.before) &&
    typeof c.before.reliable === 'boolean' && finite(c.before.meanAbsError, 0, 1e9) && Array.isArray(c.before.points) && c.before.points.length <= 20 &&
    c.before.points.every(p => record(p) && finite(p.level, 0, 1) && finite(p.measured, 0, 1e6) && finite(p.wanted, 0, 1) && finite(p.error, -1e9, 1e9));
}
