...
import { withDeadline } from './async.js?v=2.2.0';
export class DisplayStage {
  ...
  show(msg) {
    ...
    const adjustment = msg.layout === 'adjustment';
    const pattern = d.querySelector('#adjustment-pattern');
    pattern.classList.toggle('hidden', !adjustment);
    if (adjustment && !pattern.children.length) {
      for (const levels of [[0, 4, 8, 12, 16, 24, 32, 48], [208, 223, 231, 239, 247, 251, 253, 255]]) {
        const row = d.createElement('div'); row.className = 'steps';
        for (const value of levels) { const cell = d.createElement('div'), label = d.createElement('span'); cell.className = 'step'; cell.style.background = `rgb(${value},${value},${value})`; label.textContent = String(value); cell.append(label); row.append(cell); }
        pattern.append(row);
      }
    }
    ...
    for (const id of ['patch-anchor', 'patch-test']) d.querySelector('#' + id).classList.toggle('hidden', msg.layout === 'field' || adjustment);
