...
import { srgbToLinear, mean, stdev, clamp } from './colour.js?v=2.1.0';
import { abortError, withDeadline, delay, throwIfAborted } from './async.js?v=2.1.0';
import { project, validQuad } from './alignment.js?v=2.1.0';
...
export class Sensor extends EventTarget {
  constructor(video, canvas) {
    ...
    this._frameId = null; this._generation = 0; this._watchdog = null;
  }
  ...
  stop() {
    this._generation++; clearInterval(this._watchdog); this._watchdog = null;
    this.running = false;
  ...
  _loop() {
    this._lastCallbackAt = performance.now(); this._lastStallNotice = 0;
    const draw = (metadata, fallback = false) => {
      if (!this.running || this.video.readyState < 2) return;
      const mediaTime = metadata?.mediaTime ?? this.video.currentTime;
      if (fallback) {
        const presented = this.video.getVideoPlaybackQuality?.().totalVideoFrames;
        if (presented > 0 && presented === this._lastPresented) return;
        if (presented > 0) this._lastPresented = presented;
      }
      // The fallback reads only advancing media time, never repeated wall time.
      if (mediaTime <= this._lastMediaTime + 0.00001) return;
      ...
      } catch (error) { this.stop(); this.dispatchEvent(new CustomEvent('error', { detail: error })); }
    };
    const tick = (_now, metadata) => {
      if (!this.running) return;
      this._lastCallbackAt = performance.now(); draw(metadata);
      if (this.running) this._frameId = this.video.requestVideoFrameCallback
    ...
    this._watchdog = setInterval(() => {
      ...
      if (performance.now() - this._lastCallbackAt > 450) draw(null, true);
      ...
      if (age > 2500 && performance.now() - this._lastStallNotice > 3000) {
        ...
        this.dispatchEvent(new CustomEvent('stall', { detail: new Error('No fresh camera frames. Keep Safari and its camera preview visible, then retry camera setup.') }));
      }
    }, 150);
  }
