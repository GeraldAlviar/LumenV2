...
import { TestRunner } from '../js/runner.js?v=2.1.0';
import { MODES, verifyResume, patchKey } from '../js/modes.js?v=2.1.0';
import { syntheticReadings } from '../js/demo.js?v=2.1.0';
import { createSession, parseSession } from '../js/session.js?v=2.1.0';
import { SessionStore } from '../js/storage.js?v=2.1.0';
import { withDeadline, delay } from '../js/async.js?v=2.1.0';
import { validMessage, validateResults } from '../js/validation.js?v=2.1.0';
import { squareGuide, sampleRegions, project, validQuad, detectSquare, alignmentAdvice, movementAmount } from '../js/alignment.js?v=2.1.0';
import { makeReport } from '../js/report.js?v=2.1.0';
import { MotionGuard } from '../js/motion.js?v=2.1.0';
...
function savedQuick(count) {
  return createSession({ readings: syntheticReadings(quick.build().slice(0, count)), target: 'gamma22', run: 'quick', planVersion: 1,
...
test('clipped samples are rejected, never counted as completed measurements', async () => {
  ...
  assert.match(f.runner.session.reason, /clipping/);
});
...
test('altered, finished and mismatched test plans cannot be silently resumed', () => {
  ...
  assert.throws(() => verifyResume({ ...savedQuick(0), planVersion: 2 }), /cannot be resumed/);
